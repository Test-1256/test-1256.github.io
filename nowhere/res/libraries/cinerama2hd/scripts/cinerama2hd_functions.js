...
ABC.MEDIA.cinerama = function() {
	...
	var getSrcType = function (src) {
		...
		var videoExtensionsList = [ "flv", "mp4" , "f4v", "m4v","f4m"];
	...
	var  getEmbedObj = function() {
		...
		embedObj.captionSrc = cinerama.captionSrc || null;
		...
		embedObj.allowHD=true;
		embedObj.authPath="http://tviview.abc.net.au/auth/flash";
	...
	// PUBLIC
	return {
		getSrcUrl: function(playerId) {
			if(typeof getSwfById != 'function' )
			{
				return false;
			}
			try
			{
				return(getSwfById(playerId).getSrcUrl());
			}
			catch(err)
			{
				log(err);
		...
		playVideo : function(playerId, src, title) {
			...
			if(src && src != _embedObjects[playerId].src){
		...
		setLive : function(playerId, bool) {
			this.live = bool;
			if (getSwfById(playerId) && getSwfById(playerId).live) {
